...
public class ResourceManager implements Closeable {
    ...
    /**
     * Adds a new resource with the manager.
    ...
    /**
     * Return a resource from the manager.
     ...
     * @return The resource
     */
    public synchronized <ResourceT> ResourceT getResource(Class<ResourceT> resourceClass, Common.ResourceName name) {
        if (resources.containsKey(name)) {
        ...
        if (shortToLongName.containsKey(name.getName()) && shortToLongName.get(name.getName()).size() == 1) {
    ...
    /**
     * Close the resource manager by removing all resources.
     * Please note that any errors will not throw an exception. Errors will still be logged.
     */
    @Override
