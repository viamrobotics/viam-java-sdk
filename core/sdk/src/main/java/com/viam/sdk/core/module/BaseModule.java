...
import static com.viam.sdk.core.Subtype.RESOURCE_TYPE_COMPONENT;
import static com.viam.sdk.core.Subtype.RESOURCE_TYPE_SERVICE;
...
import com.viam.sdk.core.ResourceCreator;
import com.viam.sdk.core.ResourceRegistration;
import com.viam.sdk.core.RobotClient;
import com.viam.sdk.core.Server;
import com.viam.sdk.core.Subtype;
import com.viam.sdk.core.Utils;
...
public abstract class BaseModule extends ModuleServiceGrpc.ModuleServiceImplBase {
    ...
    public BaseModule(final String[] args) {
        ...
        Thread.setDefaultUncaughtExceptionHandler((thread, throwable) -> LOGGER.severe("[ERROR] Uncaught exception: " + throwable));
    ...
    public synchronized void addResource(AddResourceRequest request) {
        final Map<Common.ResourceName, Resource> deps = getDependencies(new HashSet<>(request.getDependenciesList()));
        final ComponentConfig config = request.getConfig();
        final Subtype subtype = Subtype.fromString(config.getApi());
        final Model model = Model.fromString(config.getModel());
    ...
    public void reconfigureResource(ReconfigureResourceRequest request) {
        // TODO(erd): implement
    }

    public void removeResource(RemoveResourceRequest request) {
        // TODO(erd): implement
    }

    public synchronized ReadyResponse ready(ReadyRequest request) {
        setParentAddress(request.getParentAddress());

        final Map<protoServiceSubtypePair, List<Model>> svcNameToModels = new HashMap<>();
        for (final String subtypeModelStr : Registry.registeredResourceCreators().keySet()) {
            final String[] subtypeModeLParts = subtypeModelStr.split("/");

            // TODO(erd): just store this as a equitable struct to avoid a reparse?
            final Subtype subtype = Subtype.fromString(subtypeModeLParts[0]);
            final Model model = Model.fromString(subtypeModeLParts[1]);

            final ResourceRegistration<?> registration = Registry.lookupSubtype(subtype);
            final String serviceName = registration.getProtoServiceName();

            final protoServiceSubtypePair pair = new protoServiceSubtypePair(serviceName, subtype);
