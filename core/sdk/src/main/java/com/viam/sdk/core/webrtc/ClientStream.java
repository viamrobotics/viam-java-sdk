...
public class ClientStream<RequestT, ResponseT> extends io.grpc.ClientCall<RequestT, ResponseT> {
    ...
    private synchronized void processMessage(final Grpc.ResponseMessage msg) {
        ...
        final ResponseT resp = this.methodDescriptor.getResponseMarshaller().parse(new ByteArrayInputStream(Bytes.toArray(result)));
    ...
    private synchronized void processTrailers(Grpc.ResponseTrailers trailers) {
        ...
        this.baseStream.closeWithRecvError(new Exception(String.format("Code=%d Message=%s", trailers.getStatus().getCode(), trailers.getStatus().getMessage())));
    }
    ...
    private static Grpc.Metadata.Builder fromGRPCMetadata(Metadata metadata) {
        ...
        for (String key : metadata.keys()) {
            final Iterable<String> values = metadata.getAll(Metadata.Key.of(key, Metadata.ASCII_STRING_MARSHALLER));
            if (values == null) {
                continue;
            }
    ...
    private static Metadata toGRPCMetadata(final Grpc.Metadata metadata) {
        ...
        metadata.getMdMap().forEach(
                (key, mdValues) -> mdValues.getValuesList().forEach(
                        (final String value) -> result.put(Metadata.Key.of(key, Metadata.ASCII_STRING_MARSHALLER), value)));
        return result;
    }
}
